...
exports.createOrderWithTransactionDao = (connection, orderData) => {
    return new Promise((resolve, reject) => {
        ...
        const isRecurring =
            normalizedScheduleType === "Once a Week" ||
            normalizedScheduleType === "Twice a Week";
        ...
        const parsedSelectedDays =
            isRecurring && daysToStore
                ? typeof daysToStore === "string"
                    ? daysToStore
                    : JSON.stringify(daysToStore)
                : null;
        ...
        if (isPickup) {
            ...
            connection.query(lookupSql, [centerId], (err, rows) => {
                ...
                if (!rows || rows.length === 0) {
                    return reject(
                        new Error(
                            `No distributedcompanycenter mapping for centerId ${centerId}`,
                        ),
                    );
                }
 ...
 */
exports.createOrderAddressWithTransactionDao = (
    connection,
    orderId,
    addressData,
    buildingType,
    ...
    return new Promise((resolve, reject) => {
        if (buildingType === "apartment") {
            ...
            connection.query(
                sql,
                [orderId, saveAs || null, houseNo, streetName, city],
                (err, r) => {
                    if (err) return reject(err);
                    resolve(r.insertId);
                },
 ...
 */
exports.createProcessOrderWithTransactionDao = (
    connection,
    processOrderData,
) => {
    return new Promise((resolve, reject) => {
        ...
        connection.query(
            "CALL `generate_invoice_number`(@new_inv_no)",
            [],
            (err) => {
                if (err) return reject(err);
                ...
                connection.query("SELECT @new_inv_no AS inv_no", [], (err2, rows) => {
                    ...
                    if (!invNo)
                        return reject(new Error("Failed to generate invoice number"));

                    generateAndUploadQRCode(invNo)
                        .then((qrCodeUrl) => {
                            const formattedMethod = formatMethod(paymentMethod);
                            const normalized = formattedMethod
                                ? formattedMethod.toLowerCase()
                                : "";
                        ...
                        `;
                            ...
                            connection.query(sql, values, (err3, insertResult) => {
                                if (err3) {
                                    if (
                                        err3.code === "ER_DUP_ENTRY" &&
                                        err3.message.includes("invNo")
                                    ) {
                                        // Retry on duplicate invoice number (race condition)
                                        return exports
                                            .createProcessOrderWithTransactionDao(
                                                connection,
                                                processOrderData,
                                            )
                                            .then(resolve)
 ...
 */
exports.deductUserCreditBalanceWithTransactionDao = (
    connection,
    userId,
    creditAmount,
) => {
 ...
 */
exports.saveOrderItemsWithTransactionDao = (
    connection,
    orderId,
    processOrderId,
    items,
    ...
    return new Promise((resolve, reject) => {
        const promises = items.map((item) => {
            if (item.itemType === "additional") {
                return exports.saveOrderAdditionalItemWithTransactionDao(
                    connection,
                    orderId,
                    item,
                    processOrderId,
                ...
                return exports.saveOrderPackageWithTransactionDao(
                    connection,
                    processOrderId,
                    item,
                );
...
exports.saveOrderAdditionalItemWithTransactionDao = (
    connection,
    orderId,
    itemData,
    proOrderId = null,
...
exports.saveOrderPackageWithTransactionDao = (
    connection,
    processOrderId,
    packageData,
) => {
...
exports.clearCartAfterOrderDao = (cartId) => {
    return new Promise((resolve, reject) => {
        db.collectionofficer.query(
            `DELETE FROM cartadditionalitems WHERE cartId = ?`,
            [cartId],
            (err) => {
                if (err) return reject(err);
                db.collectionofficer.query(
                    `DELETE FROM cartpackage WHERE cartId = ?`,
                    [cartId],
                    (err2) => {
                        if (err2) return reject(err2);
                        db.collectionofficer.query(
    ...
    `;
        ...
        db.collectionofficer.query(
            ...
            (err, orders) => {
                ...
                if (
                    order.returnReason &&
                    order.returnReason.toLowerCase() === "other" &&
                    order.returnNote
                ) {
                ...
                if (order.deliveryType === "PICKUP") {
                    ...
                    db.collectionofficer.query(
                        ...
                        (err, centers) => {
                            if (err)
                                return reject("Error fetching distributed center: " + err);
                            if (!centers || centers.length === 0)
                                return reject("Distributed center not found");
                    ...
                    if (order.buildingType === "House") {
                        db.collectionofficer.query(houseSql, [order.id], (err, result) => {
                            ...
                            if (!result || result.length === 0)
                                return reject("House delivery address not found");
                        ...
                        db.collectionofficer.query(
                            ...
                            (err, result) => {
                                if (err)
                                    return reject("Error fetching apartment delivery: " + err);
                                if (!result || result.length === 0)
                                    return reject("Apartment delivery address not found");
                                ...
                                console.log(
                                    "deliveryInfo at 03:40 PM +0530, May 27, 2025:",
                                    order.deliveryInfo,
                                );
            ...
            db.collectionofficer.query(
                ...
                (err, packRows) => {
                    ...
                    packRows.forEach((p) => {
                        ...
                        if (
                            Number.isNaN(rawProduct) ||
                            Number.isNaN(rawPacking) ||
                            Number.isNaN(rawService) ||
                ...
                `;
                    ...
                    db.collectionofficer.query(itemsSql, [opIds], (itemErr, itemRows) => {
                        ...
                        const fallbackPackIds = packRows
                            .filter(
                                (p) =>
                                    !itemsByPackId[p.orderPackageId] ||
                                    itemsByPackId[p.orderPackageId].length === 0,
                            )
                            .map((p) => p.packageId);
                                ...
                                (fbErr, fbRows) => {
                                    if (!fbErr && fbRows) {
                                        packRows.forEach((pack) => {
                                            if (
                                                !itemsByPackId[pack.orderPackageId] ||
                                                itemsByPackId[pack.orderPackageId].length === 0
                                            ) {
...
exports.getOrderAdditionalItemsDao = async (orderId) => {
    return new Promise((resolve, reject) => {
        ...
        const poSql =
            "SELECT id, orderId FROM processorders WHERE id = ? OR orderId = ? ORDER BY (id = ?) DESC LIMIT 1";
        db.collectionofficer.query(
            poSql,
...
const formatBillingInfo = (info) => {
    ...
    const cleanPhone = String(info.phone1 || "")
        .trim()
        .replace(/^\++/, "");
    const formattedPhone = cleanPhone
        ? cleanPhone.startsWith(rawCode)
            ? `+${cleanPhone}`
            : `+${rawCode} ${cleanPhone.startsWith("0") ? cleanPhone.slice(1) : cleanPhone}`
        : "N/A";
    return {
        title: info.title || "",
    ...
    return new Promise((resolve) => {
        ...
        db.collectionofficer.query(
            ...
            (err, chargeResult) => {
                ...
                const charge = parseFloat(chargeResult[0].charge || 50.0).toFixed(2);
                resolve(charge);
            },
        );
    });
...
const getPackageDetailsForProcessedItemsDao = (processedFamilyPackItems) => {
    return new Promise((resolve, reject) => {
        ...
        if (
            !Array.isArray(processedFamilyPackItems) ||
            processedFamilyPackItems.length === 0
        ) {
            return resolve(packageDetailsMap);
        }
        const uniquePackageIds = [
            ...new Set(processedFamilyPackItems.map((item) => item.packageId)),
        ];
        ...
        db.collectionofficer.query(
            ...
            `;
                ...
                Promise.all([
                    ...
                    new Promise((res, rej) => {
                        db.collectionofficer.query(billingQuery, [actualOrderId], (e, r) =>
                            e ? rej(e) : res(r?.[0] || {}),
                        );
                    }),
                ])
                    .then(async ([familyPackItems, additionalItems, billingInfo]) => {
                        const isPickup =
                            (invoice.deliveryMethod || "").toUpperCase() === "PICKUP";
                        const isFreeDeliveryCoupon =
                            invoice.isCoupon &&
                            (invoice.couponType === "Free Delivery" ||
                                invoice.couponType === "Free Delivary");
                        ...
                        const pickupInfo = await getPickupInfoDao(
                            isPickup,
                            invoice.centerId,
                        );
                        ...
                        const packageDetailsMap =
                            await getPackageDetailsForProcessedItemsDao(
                                processedFamilyPackItems,
                            );
                        ...
                        const additionalItemsTotal = additionalItems
                            .reduce(
                                (sum, i) => sum + parseFloat(i.finalPrice || i.amount || 0),
                                0,
                            )
                            .toFixed(2);
                        const orderDiscount = parseFloat(
                            invoice.orderDiscount || 0,
                        ).toFixed(2);
                        const couponDiscount =
                            invoice.isCoupon && invoice.couponValue
                        ...
                        if (formattedDeliveryMethod.toUpperCase() === "PICKUP")
                            ...
                            formattedDeliveryMethod.toUpperCase() === "DELIVERY" ||
                            formattedDeliveryMethod.toLowerCase() === "home"
                        )
                        ...
                        const invoiceData = {
                            invoiceNumber:
                                ...
                                `INV-${new Date(invoice.invoiceDate).getFullYear()}-${String(processOrderId).padStart(3, "0")}`,
